...
abstract contract Rental is Ownable, IERC721Receiver {
    ...
    constructor(
        ...
        if (_borrowerAddress.balance < rentalPayment + collateral) revert InsufficientValue();
    ...
    function depositNft() external payable {
        ...
        if (!ethIsDeposited) {
            ...
            nft.transferFrom(msg.sender, address(this), nftId);
            nft.transferFrom(address(this), borrowerAddress, nftId);
    ...
    function depositEth() external payable {
        ...
        if (block.timestamp >= nullificationTime) {
            if (nft.ownerOf(nftId) == address(this)) {
                nft.transferFrom(address(this), lenderAddress, nftId);
            }
            selfdestruct(payable(borrowerAddress));
        }
